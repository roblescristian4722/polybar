...
#trap 'exit' SIGINT
  ...
  upgrade() {
    if [ -s ${path}repo.pkgs ]; then
      [ -s ${path}aur.pkgs ] && gnome-terminal -e "aur sync -c -u --noview && sudo pacman -Syu --noconfirm" || \
                                gnome-terminal -e "sudo pacman -Syu --noconfirm"
      echo "%{T7}0%{T-}" > ${path}status && >| ${path}repo.pkgs && >| ${path}aur.pkgs
    elif [ -s ${path}aur.pkgs ]; then
      gnome-terminal -e "aur sync -c -u --noview && sudo pacman -Syu --noconfirm"
      echo "%{T7}0%{T-}" > ${path}status && >| ${path}repo.pkgs && >| ${path}aur.pkgs
    else
  ...
  vcs_upgrade() {
    ...
    [ -s ${path}aur-vcs.pkgs ] && gnome-terminal -e bash -ci \
  ...
  clean() {
    gnome-terminal -e sh -c "clean_aurutils_cache && sudo pacman -Sc"
  }
